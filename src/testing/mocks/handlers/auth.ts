// for the auth endpoint we will get the following endpoints
// POST /auth/login
// POST /auth/logout
// GET /auth/me
...
const loginHandler = rest.post(
  `${API_URL}/auth/login`,
  async (req, res, ctx) => {
    const credentials = await req.json(); // parse request body
...
const meHandler = rest.get(`${API_URL}/auth/me`, (req, res, ctx) => {
