  ...
  users: [
    {
      ...
      password: "password",
      organisationId: "amYXmIyT9mD9GyO6CCr"
    }
  ],
  organisations: [
    {
...
export const getOrganisation = delayedFn(
  (id: string) => testData.organisations.find(o => o.id === id) || null,
  300
);

export const getJobs = delayedFn(
  (organisationId: string) =>
    testData.jobs.filter(j => j.organisationId === organisationId),
  300
);
...
export const useOrganisation = (id: string) => useTestData(getOrganisation(id));

export const useJobs = (organisationId: string) =>
  useTestData(getJobs(organisationId));
